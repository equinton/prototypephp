...
foreach ( $menuarray as $key => $value ) {
	 ...
	 */
	if ($ok) {
		$menu .= '<li id="' . $value ["module"] . '"><a href="index.php?module=' . $value ["module"] . '" title="' . $LANG ["menu"] [$value ["menutitle"]] . '">' . $LANG ["menu"] [$value ["menuvalue"]] . '</a>';
		$flag = 0;
		/*
		 * Gestion sous-menu
		 */
		...
		foreach ( $value as $key1 => $value1 ) {
			if (is_array ( $value1 )) {
				...
				if ($ok1) {
					...
					$menu .= '<li id="' . $value1 ["module"] . '"><a href="index.php?module=' . $value1 ["module"] . '" title="' . $LANG ["menu"] [$value1 ["menutitle"]] . '">' . $LANG ["menu"] [$value1 ["menuvalue"]] . '</a></li>';
				}
			}
		}
		if ($flag == 1)
